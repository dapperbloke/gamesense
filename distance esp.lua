...
local esp = {
    ...
    weaponnamescolour = ui.new_color_picker('visuals', 'player esp', 'Weapon names', 255,255,255,255),
        meters = ui.new_slider('visuals', 'player esp', 'Maxmimum Distance', 1, 100, 20, true, 'M')
}
...
local function drawName()
    ...
    local players = globals.maxplayers()
    for i = 1, players do
        if entity.is_dormant(i) then goto stop end
        local x, y, z = entity.get_prop(i, "m_vecOrigin")
        local x1, y1, z1 = entity.get_prop(entity.get_local_player(), "m_vecOrigin")
        local dist = math.sqrt(math.pow(x - x1) + math.pow(y - y1))
        local meters = dist/39.37
        if meters > ui.get(esp.meters) then goto stop end
        local resource = entity.get_player_resource()
...
local function drawBoxes()
    ...
    for i = 1, players do
        ...
        local x1, y1, z1 = entity.get_prop(entity.get_local_player(), "m_vecOrigin")
        local dist = math.sqrt(math.pow(x - x1) + math.pow(y - y1))
        local meters = math.abs(dist/39.37)
        if meters > ui.get(esp.meters) then goto stop end
        local resource = entity.get_player_resource()

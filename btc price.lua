...
function btccounter()
    ...
    renderer.text(x+w/2, y+h-20, 255, 255, 255, 255, "cb", 0, "$"..cryptoparsed.bpi.USD.rate)
    renderer.texture(coinenz, x+w/2-50, y+h/2-60, 100, 100, 255, 255, 255, 255)
    ...
    if client.key_state(0x01) and mouseposy <= y+h and mouseposy >= y and mouseposx <= x+w and mouseposx >= x then
        x = mouseposx - w/2
        database.write("cryptoposx", mouseposx - w/2)
        y = mouseposy - h/2
        database.write("cryptoposy", mouseposy - h/2)
    end
end
http.get("https://api.coindesk.com/v1/bpi/currentprice.json", function(success, response)
    if not success or response.status ~= 200 then
      return
    end
    cryptoparsed = json.parse(response.body)
    client.set_event_callback("paint_ui", btccounter)
end)
function refresher()
    ui.set(ui.reference("CONFIG", "Lua", "Reload active scripts"), true)
end
client.set_event_callback("round_start", refresher)
